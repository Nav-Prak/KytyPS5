...
void TestValidAndHostileMetadata() {
  ...
  CheckRejected(bad_semantics.mapped,
                "unreadable vertex semantic array was accepted");
}
...
void TestCrossPageMetadata() {
  ...
  DWORD old_protect = 0;
  Check(VirtualProtect(pages + page_size, page_size, PAGE_NOACCESS,
                       &old_protect) != 0,
  ...
  CheckRejected(header_crossing,
                "cross-page AGC user-data header was accepted");
  ...
  CheckRejected(offsets_crossing,
                "cross-page direct-resource offsets were accepted");
  ...
  CheckRejected(semantics_crossing,
                "cross-page vertex semantics were accepted");

#if KYTY_PLATFORM == KYTY_PLATFORM_WINDOWS
  Check(VirtualFree(pages, 0, MEM_RELEASE) != 0,
        "failed to free metadata test pages");
#elif KYTY_PLATFORM == KYTY_PLATFORM_LINUX
