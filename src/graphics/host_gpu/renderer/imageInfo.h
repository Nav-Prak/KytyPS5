...
IsKnownCompressedVideoOut(VideoOutCompression compression) noexcept {
	return compression == VideoOutCompression::Dcc256_64_64 ||
	       compression == VideoOutCompression::Dcc256_256_0;
}
...
IsSupportedStandard64RenderTarget(const RenderTargetInfo& info) noexcept {
	if (info.tile_mode != Prospero::GpuEnumValue(Prospero::TileMode::kStandard64KB) ||
	    ...
	    info.bytes_per_element != 4 || info.levels != 1 || info.layers == 0) {
		return false;
	}
	...
	if (expected_pitch > UINT32_MAX || info.pitch != expected_pitch ||
...
[[nodiscard]] inline bool IsCompatibleStorageSampledViewFormat(vk::Format cached,
                                                               vk::Format requested) noexcept;
[[nodiscard]] inline bool IsMutableStorageSampledViewFormat(vk::Format format) noexcept;
...
[[nodiscard]] inline bool IsCompatibleStorageSampledViewFormat(vk::Format cached,
	...
	return cached == requested || IsRgba8SrgbReinterpretation(cached, requested) ||
	       ...
	       (cached == vk::Format::eR8Uint && requested == vk::Format::eR8Unorm) ||
	       (cached == vk::Format::eR8Unorm && requested == vk::Format::eR8Uint);
}

[[nodiscard]] inline bool IsMutableStorageSampledViewFormat(vk::Format format) noexcept {
	return IsRgba8SrgbViewFormat(format) || format == vk::Format::eR8Uint ||
	       format == vk::Format::eR8Unorm || format == vk::Format::eR32Uint ||
